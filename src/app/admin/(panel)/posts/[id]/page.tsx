    ...
    return (
        <div className="space-y-6 max-w-4xl">
            <div className="flex items-center gap-4">
                <Button variant="ghost" onClick={() => router.push("/admin/posts")}><ArrowLeft
                    className="w-4 h-4 mr-2"/>Back</Button>
                <div className="flex-1"><h1
                    className="text-3xl font-heading font-bold">{isNew ? "New Post" : "Edit Post"}</h1></div>
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                        <Switch id="published" checked={formData.is_published}
            ...
            {/* Display Controls */}
            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2"><Settings2 className="w-5 h-5"/>Display
                    Controls</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                    <div className="space-y-3">
                        <Label>Where to display this post:</Label>
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2">
                                <Checkbox id="offers_page" checked={formData.display_locations.includes("offers_page")}
                                          onCheckedChange={() => toggleDisplayLocation("offers_page")}/>
                                <Label htmlFor="offers_page" className="font-normal">Show on Offers page</Label>
                            </div>
                            <div className="flex items-center gap-2">
                                <Checkbox id="home_latest" checked={formData.display_locations.includes("home_latest")}
                                          onCheckedChange={() => toggleDisplayLocation("home_latest")}/>
                                <Label htmlFor="home_latest" className="font-normal">Show on Home "Latest
                                    Offers"</Label>
                            </div>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="flex items-center gap-3 pt-6">
                            <Switch id="pinned" checked={formData.pinned}
            ...
            {/* Images */}
            <Card>
                <CardHeader><CardTitle className="flex items-center gap-2"><ImageIcon
                    className="w-5 h-5"/>Images</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                    <ImageUploader label="Cover Image" value={formData.cover_image_url}
                                   onChange={(url) => setFormData({...formData, cover_image_url: url})}/>
                    <GalleryUploader label="Gallery Images" value={formData.gallery}
                        ...
                        {["en", "ka", "ru"].map((lang) => (
                            <TabsContent key={lang} value={lang} className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Input value={formData[`title_${lang}` as keyof PostFormData] as string}
                                           onChange={(e) => handleTitleChange(lang, e.target.value)}/>
                                </div>
                                <div className="space-y-2">
                                    <Label>Excerpt ({lang.toUpperCase()})</Label>
                                    <Textarea value={formData[`excerpt_${lang}` as keyof PostFormData] as string}
                                              onChange={(e) => setFormData({
                                                  ...formData,
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Textarea value={formData[`content_${lang}` as keyof PostFormData] as string}
                                              onChange={(e) => setFormData({
                                                  ...formData,
